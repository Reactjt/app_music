...
useEffect(() => {
  ...
  const initializeWaveSurfer = () => {
    console.log(waveformContainerRef.current.id)
    ...
    if (!waveSurferRef.current && waveformContainerRef.current) {
      ...
      // Load the audio for the current song if available
      if (currentSong) {
  ...
  // Check if the document is ready, and if so, initialize WaveSurfer
  ...
  return (
    ...
    <div ref={waveformContainerRef} id={waveformid} className="w-1/2 mb-8"></div>         
