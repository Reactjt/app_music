...
export default function Waveforms({ filteredinfo, audioUrl, id, waveforms, setWaveforms, song }) {
   ...
   useEffect(() => {
      ...
      ws.on("ready", () => {
         ...
         // ws.setVolume( 1 )
         ws.on('click', updateTimeOnInteraction )
         // ws.pause()
      });
   ...
   //    waveform.on( 'interaction', updateTimeOnInteraction )
   //    // console.log( waveform );

   // }, [song])

   function updateTimeOnInteraction() {
      const waveform = waveforms[id];
      if (!waveform || song !== id) return
      setCurrentTimestamp( waveform.getCurrentTime() );
