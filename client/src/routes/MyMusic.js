...
const SingleFilterCard = ({ info }) => {
  ...
  const { selectedFilter } = useContext(filteredInfoContext);
  ...
  useEffect(() => {
    const initializeWaveform = (audioUrl, id) => {
      const ws = WaveSurfer.create({
        container: `#waveform-${id}`, // Use a unique identifier for each waveform container
        responsive: true,
        barWidth: 2,
        barHeight: 4,
        barGap: null,
  ...
  return (
    <div>
      {songs.map((item, index) => (
        <div
          key={index}
          className={`p-2 ${scrollY > 200 ? "song-fade" : ""}`}
        >
          <div className=" hover:bg-gray-400 hover:bg-opacity-20 p-4  flex items-center  border-b-gray-200 border-b border-opacity-10">
            <div className="flex items-center">
              ...
              <div className="mx-6 md:ml-6">
                <button
                  onClick={() => playSong(item.audio, index)}
